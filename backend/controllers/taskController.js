...
const updateTask = async (req, res) => {
    try {
        ...
        task.todoChecklist = req.body.todoChecklist || task.todoChecklist; task.attachments = req.body.attachments ||
            task.attachments;
        ...
        if (req.body.assignedTo) {
            ...
            res.json({ message: "Task updated syccessfully", updateTask });
        }
    } catch (error) {
        res.status(500).json({ message: "Server error", error: error.message });
    }
...
const updateTaskChecklist = async (req, res) => {
    try {
        ...
        // Auto-update progress based on checklist copmletion
        const completedCount = task.todoChecklist.filter(
            (item) => item.completed
        ).length;
        ...
        // Auto-mark task completed if all items are checked
...
const getDashboardData = async (req, res) => {
    try {
        ...
        const taskPriorityLevels = taskPrioRities.reduce((acc, priority) => {
            acc[priority] =
                taskPriorityLevelsRaw.find((item) =>
